...
import com.seeingvoice.www.svhearing.smsverify.VerifyPopupWindow;
...
//        Log.e(TAG, "onReq:" + baseReq.openId);
    ...
    public void onResp(BaseResp baseResp) {
        String result;
        if (baseResp instanceof SendAuth.Resp) {//微信授权登录
            switch (baseResp.errCode) {
                case BaseResp.ErrCode.ERR_OK://用户同意授权
                    ...
                    //get请求发送code到
                    mOkHttpManager.getNet(AppConstant.SEND_WECHAT_CODE + code, new OkHttpManager.ResultCallback() {
                        @Override
                        public void onFailed(Request request, IOException e) {
                            Log.e(TAG, "得到耿的信息失败" + request.toString());
//                            mVerifyPopupWindow.showPopupWindow();//测试完成后，别忘记删除掉
                            showDialog("网络错误");
                        }
    ...
    public void checkWXLogin(String response) {
        ...
        if (messageCode.equals(AppConstant.NET_STATE_SUCCESS)) {
            MyApplication.setLoginSuccess(true, AppConstant.WECHAT_LOGIN, mUserInfoBean.getUid(), mWechatLoginInfoBean, System.currentTimeMillis());
