...
public class ShareDialog extends BaseDialog{
    ...
    private String[]  mTexts={"QQ好友","QQ空间","微信好友","微信朋友圈"};
    private int[]	  mImgs={R.mipmap.qq, R.mipmap.kongjian,

            R.mipmap.weixin,R.mipmap.pengyouquan};//图片数据
    ...
    private View initView() {
        ...
        for(int i=0,j=0;i<num;){
            ...
            for(;i<j*number && i<num;i++){//一行三个
                //设置分享图文控件
                mTextView item=new mTextView(mContext);
                params=new LinearLayout.LayoutParams(
                        mScreenWidth/number,
                        LinearLayout.LayoutParams.MATCH_PARENT
                );
