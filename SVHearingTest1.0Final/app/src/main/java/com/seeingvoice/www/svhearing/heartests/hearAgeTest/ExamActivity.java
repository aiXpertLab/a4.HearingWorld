...
public class ExamActivity extends TopBarBaseActivity {
    ...
    private playingThreadtest mPlayingThread;
    ...
    protected void init(Bundle savedInstanceState) {
        ...
        setToolBarMenuTwo("", R.mipmap.jiaocheng, new OnMenuClickListener() {
            @Override
            public void onMultiClick(MenuItem v) {
                toNextActivity(null, ExamActivity.this, AboutUsActivity.class);
            }
        });
        ...
        count = list.size();
        current = 0;
        mPlayingThread = new playingThreadtest(list.get(current).frequency_band,40);
    ...
    private void toNextQuestion(final int updatecurrent) {
        stopPlaying();
        mPlayingThread = new playingThreadtest(list.get(updatecurrent).frequency_band,40);
        mPlayingThread.start();
        isPlaying = true;
        //为控件赋值
