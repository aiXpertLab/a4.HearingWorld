...
public class L_PlayingPureTone extends Thread {
    ...
    private final short[] msin;
    private float[] mFsin;
    public static boolean ISPLAYSOUND;
    private final int bufferSize;
    private final int length;
    private final float waveLen;
    public static final boolean LEFT = true;
    public static final boolean RIGHT = false;
    Context mContext = MyApp.getAppContext();
    private SettingsContentObserver mSettingsContentObserver;

    public L_PlayingPureTone(int Hz, int dB){
...
//        mFsin = UpdateSinWave.getFloatSinWave(waveLen,length);
     ...
     */
    public L_PlayingPureTone(boolean isleft, int Hz, int dB) {
        initSampleRate();
        if (RATE == 0) RATE = 44100; // Use a default value if property not found
        float volume = MyApp.volume;//获得系统音频流音量  system_stream  默认就是最大值  无需调节
