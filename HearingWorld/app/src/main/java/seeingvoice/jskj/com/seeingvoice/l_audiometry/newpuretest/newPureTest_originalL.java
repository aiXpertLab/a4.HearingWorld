...
/** 标题栏设置*/
    ...
    private void initVolume() {
        int haldMaxVol = L_AudioManager.getInstance(newPureTest_originalL.this).getMaxMediaVolume()/2;
        L_AudioManager.getInstance(newPureTest_originalL.this).setMediaVolume(haldMaxVol);
    }
    ...
    private boolean isWirelessOn() {
        if (null != BlueAdapter){
            ...
            if (BluetoothProfile.STATE_CONNECTED == state) {
                microSpeaker(newPureTest_originalL.this);                // 蓝牙设备已连接，声音内放，从蓝牙设备输出
                ...
                loudSpeaker(newPureTest_originalL.this);                // 蓝牙设备未连接，声音外放，
                return false;
            }
        }else {
//            mHeadsetConnHintText.setText("该手机不支持蓝牙功能~");
                        ...
                        if (isCurrentTestLeft) {//当前测试左耳
                            ...
                            if (isLeftFinished) {//左耳完成测试
                                ...
                                if (btnCheckResult.getVisibility() == View.VISIBLE) {//最后出测试结果了，不应该再提示 切换了
                                    ...
                                    AlertDialog.Builder builder = new AlertDialog.Builder(newPureTest_originalL.this);
    ...
    private void toResult() {    //测试完成进入结果页面
        ...
        /* 送数据到结果页面*/
        Intent intent = new Intent(newPureTest_originalL.this, L5_ResultT4.class);
