...
public class BleMsgUtil {
    ...
    public static void loginRes(SPPLinkUtil2 sppUtil) {
        ...
        resultStr.append("7f").append(L_CommonUtil.specialCharReplace(replaceStr+ConvertData.makeChecksum(contentStr.toString()))).append("f7");
        byte[] bytes = ConvertData.hexStringToBytes(resultStr.toString());
        L_LogUtils.d(TAG,"登录发送给眼镜数据："+ConvertData.bytesToHexString(bytes,false));
    ...
    public static void rotateScreen(SPPLinkUtil2 sppUtil){
        ...
        L_LogUtils.d(TAG,"旋转左右！"+ConvertData.bytesToHexString(bytes, false));
        sendMsgBle(bytes,sppUtil);
    }
